...
class MistakeVoter extends Voter
{
    const MISTAKE_VIEW_ROLE = 'MISTAKE_VIEW_ROLE'.__CLASS__;
    const MISTAKE_EDIT_ROLE = 'MISTAKE_EDIT_ROLE'.__CLASS__;
    const MISTAKE_ADD_ROLE = 'MISTAKE_ADD_ROLE'.__CLASS__;
    const MISTAKE_LIST_ROLE = 'MISTAKE_LIST_ROLE'.__CLASS__;

    protected function supports($attribute, $subject)
    {
        // if the attribute isn't one we support, return false
        if (!in_array($attribute, [
            self::MISTAKE_VIEW_ROLE,
            self::MISTAKE_EDIT_ROLE,
            self::MISTAKE_ADD_ROLE,
            self::MISTAKE_LIST_ROLE,
        ], true)) {
            return false;
        }
    ...
    {
        switch ($attribute) {
            case self::MISTAKE_LIST_ROLE:
                return true;
            case self::MISTAKE_VIEW_ROLE:
                return true;
            case self::MISTAKE_ADD_ROLE:
                return !in_array(User::ROLE_USER, $user->getRoles(), true);
            case self::MISTAKE_EDIT_ROLE:
                return !in_array(User::ROLE_USER, $user->getRoles(), true);
            default:
                return false;
        }
