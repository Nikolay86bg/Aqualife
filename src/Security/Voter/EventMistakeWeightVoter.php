...
class EventMistakeWeightVoter extends Voter
{
    const EVENT_MISTAKE_WEIGHT_VIEW_ROLE = 'EVENT_MISTAKE_WEIGHT_VIEW_ROLE'.__CLASS__;
    const EVENT_MISTAKE_WEIGHT_EDIT_ROLE = 'EVENT_MISTAKE_WEIGHT_EDIT_ROLE'.__CLASS__;
    const EVENT_MISTAKE_WEIGHT_ADD_ROLE = 'EVENT_MISTAKE_WEIGHT_ADD_ROLE'.__CLASS__;
    const EVENT_MISTAKE_WEIGHT_LIST_ROLE = 'EVENT_MISTAKE_WEIGHT_LIST_ROLE'.__CLASS__;

    protected function supports($attribute, $subject)
    {
        // if the attribute isn't one we support, return false
        if (!in_array($attribute, [
            self::EVENT_MISTAKE_WEIGHT_VIEW_ROLE,
            self::EVENT_MISTAKE_WEIGHT_EDIT_ROLE,
            self::EVENT_MISTAKE_WEIGHT_ADD_ROLE,
            self::EVENT_MISTAKE_WEIGHT_LIST_ROLE,
        ], true)) {
            return false;
        }
    ...
    {
        switch ($attribute) {
            case self::EVENT_MISTAKE_WEIGHT_LIST_ROLE:
                return true;
            case self::EVENT_MISTAKE_WEIGHT_VIEW_ROLE:
                return true;
            case self::EVENT_MISTAKE_WEIGHT_ADD_ROLE:
                return !in_array(User::ROLE_USER, $user->getRoles(), true);
            case self::EVENT_MISTAKE_WEIGHT_EDIT_ROLE:
                return !in_array(User::ROLE_USER, $user->getRoles(), true);
            default:
                return false;
        }
